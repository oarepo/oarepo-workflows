...
class PrivilegedAccessBase(RequestPolicyWorkflowCreators):
    ...
    def _kwargs_parser(self, **kwargs: Any) -> dict[str, Any]:
        request: Request = kwargs["request"]
        kwargs.setdefault("action", kwargs["generator_action"])
        kwargs.setdefault("request_type", request.type)
        try:
            kwargs["record"] = request.topic.resolve()
        except Exception as e:  # noqa              TODO: better exception catching here
    ...
    def needs(self, **context: Any) -> list[Need]:  # type: ignore
        ...
        try:
            context = self._kwargs_parser(**context)
            generators = self._requester_generators(**context)
            needs = [generator.needs(**context) for generator in generators]
            return [need for generator_needs in needs for need in generator_needs]
        except (MissingWorkflowError, InvalidWorkflowError, MissingTopicError):
            return []
    ...
    def excludes(self, **context: Any) -> list[Need]:
        ...
        try:
            context = self._kwargs_parser(**context)
            generators = self._requester_generators(**context)
            excludes = [generator.excludes(**context) for generator in generators]
            return [exclude for generator_excludes in excludes for exclude in generator_excludes]
        except (MissingWorkflowError, InvalidWorkflowError, MissingTopicError):
            return []
...
class PrivilegedAccess(PrivilegedAccessBase):
    ...
    def _requester_generators(self, **kwargs: Any) -> list[Generator]:
        ...
        return workflow_request.privileged
...
class EventsPrivilegedAccess(PrivilegedAccessBase):
    ...
    def _requester_generators(self, **kwargs: Any) -> list[Generator]:
        ...
        ret = []
        privilegeds = workflow_request.privileged
        if privilegeds:
            for privileged in privilegeds:
                if privileged._events and event_type.type_id in privileged._events:
                    ret.append(privileged._role)
        return ret