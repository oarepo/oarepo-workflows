...
class FromRecordWorkflow(Generator):
    ...
    def __init__(self, action: str | Callable, record_getter: Callable | None = None) -> None:
    ...
    def _get_workflow(self, record: Record | None = None, **context: Any) -> str:
    ...
    def _get_permissions_from_workflow(
        ...
        **context: Any,
        ...
        if self._record_getter:
            record = self._record_getter(**context) # TODO: delete_draft accept action create_comment crashes here; the draft is already deleted
                                                    # we should explicitly decide what should throw exception and what should not do anything
            if not record:
                return None
        policy_cls = self._get_workflow(record, **context).permissions
        action_name = self._action_name(**context)
        policy = policy_cls(action_name, record=record, **context)
    ...
    def needs(self, **context: Any) -> Sequence[Need]:
        ...
        if policy is not None: # invenio adds disable generator if the action doesn't exist
            return self._get_permissions_from_workflow(**context).needs  # type: ignore[no-any-return]
        else:
    ...
    def excludes(self, **context: Any) -> Sequence[Need]:
        ...
        if policy is not None:
            return self._get_permissions_from_workflow(**context).excludes  # type: ignore[no-any-return]
        else:
            return []
    ...
    def query_filter(self, **context: Any) -> dsl.query.Query:
        ...
        policy = self._get_permissions_from_workflow(**context)
        if policy is not None:
            queries = self._get_permissions_from_workflow(**context).query_filters
