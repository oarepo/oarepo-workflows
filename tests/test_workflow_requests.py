...
def test_getitem_nonexistent_request_raises_error(app, search_clear):
    request_policy = current_oarepo_workflows.workflow_by_code["is_applicable_workflow"].requests()
...
def test_getitem_nonexistent_event_raises_error(app, search_clear):
    ...
    assert exc_info.value.workflow == "is_applicable_workflow"
    assert exc_info.value.description == "Event type nonexistent_event not found in workflow is_applicable_workflow."
